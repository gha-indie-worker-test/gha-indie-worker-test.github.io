// OWLS build-time resource-hint helper for the pilot harness.
//
// PROVENANCE: this file is a near-verbatim copy of
//   gha-indie-worker/gha-indie-worker.github.io : src/lib/prepare.mjs
// The ONLY intended difference is DEFAULT_ORIGINS below, which points at this
// test organization's own Pages origin instead of the production subdomains.
// Keeping it a copy is deliberate: the harness must exercise the same code the
// production site runs, and neither repository may take an npm dependency on
// the other. Nothing checks this copy for drift automatically; if you change
// one, change both.
//
// WHY IT HAS NO IMPORTS
// It does NOT import @ores-wasm-loaders/owls-web-loader. It re-implements, by
// hand, the selection and budget semantics of that package's `hintDescriptors()`
...
// Origins this harness is willing to name in a resource hint.
//
// This is the ONE origin this test organization actually controls: its own
// GitHub Pages site. Both harness pages are served from it, so the reuse this
// harness measures is SAME-ORIGIN. That is a weaker case than the production
// scenario (same-site, cross-origin subdomain), and the README says so.
const DEFAULT_ORIGINS = Object.freeze(["https://gha-indie-worker-test.github.io"]);
